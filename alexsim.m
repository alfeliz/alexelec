...
#	- OBTAIN THE RESISTANCE THAT SIMULATES THE EXPLODNG WIRE CIRCUIT
...
#			 linea		To make function: A + B*cot(frec*t).
...
#Choosing the shot:
shot = "ALEX103"; %To make it unique
...
if (fileinfo == -1) 
   error ("Script alexsim: Unable to open file name: %s, %s",infoshot, msg); 
endif;
...
for i=1 : length(txtinfo)
 ...
 elseif(cell2mat(strfind(txtinfo(i),"Del")) ==1) %We should stop to avoid channels confusion.
   #After channels info cames other shot info, not stored or used in this version of alexsim.m
   break;
 endif;
endfor;
...
if (exist("volt","var")==0)
  warning("Script alexsim: There is no voltage signal in shot %s",shot);
elseif (exist("dint","var")==0)
  warning("Script alexsim: There is no intensity signal in shot %s",shot);
elseif (exist("phot","var")==0)
  warning("Script alexsim: There is no photodiode signal in shot %s",shot);
endif;
...
if (exist("volt","var")==1)
  ...
  if (puntosv<=10)
    warning("Script alexsim: only %u points in voltage baseline",puntosv);
  endif;
...
if (exist("phot","var")==1)
  ...
  if (puntosp<=10)
    warning("Script alexsim: only %u points in photodiode baseline",puntosp);
  endif;
...
if (exist("dint","var")==1)
  ...
  if (puntosp<=10)
    warning("Script alexsim: only %u points in d(i)/dt baseline",puntosp);
  endif;
...
Ltot = Lt + 118e-9;
disp("Wire total inductance: (nHr)");
...
###############################################################################plot(Vres(:,1),Vres(:,2)); %To see the resistive part of the voltage


####
# Finding the equivalente resistance to simulate the voltage from the intensity
####

res =  zeros(length(volt(:,1)),1); %resistance vector initialization with 0.
rline = linea(3, abs(dint(11,1)-dint(10,1)),0.023, 0.22, 0.7); %Resistance as function of time(µs)
id = find(gradient(volt(:,2))>20); %Put in an index vector positions of current gradient positive, to know were the current starts.

disp("Time to start resistance: (µs)")
dint(id(1),1) %Time for r(t) to start. Beginning of voltage.

res(1+id(1):length(rline)+id(1)) = rline;


valter = -( Ltot .* dint(:,2)  ) + ( res.*int ) ;

valt(:,1) = volt(:,1);
valt(:,2) = valter;

figure;
plot(valt(:,1),valt(:,2),"*r"); %Simulated voltage
hold;
plot(volt(:,1),volt(:,2),"+k"); %Measured voltage
...
if (exist("Vres","var"))
  ...
  #First line:
  fdisp(output,"descriptor `time(micros)`  `res. voltage(V)` `tot. volt(V)` `simulated tot. volt.(V)`"); %Veusz format
  redond = [2 3 3 3]; %Saved precision 
  vol = [Vres, volt(:,2), valt(:,2)];
...
if (exist("electricenergia2","var"))
  ...
  disp("Electrical energies saved.");
endif;
...
if (exist("int","var") && exist("volt","var"))
  ...
  output = fopen(name,"w"); %Opening the file.
  #First line:
  fdisp(output,"descriptor `time(micros)`  `calc. res(Ohm)` `sim. res(Ohm)`"); %Veusz format
  redond = [2 3 3]; %Saved precision
  resistance = [dint(:,1), Vres(:,2)./int, res]; 
  display_rounded_matrix(resistance, redond, output); %This function is not made by my.
  fclose(output);%Closing file
  disp("Resistances saved.");
endif;
...
###
timing = toc;
disp("Script alexsim execution time:")
disp(timing)
disp(" seconds")
